...
impl NativeFungibleTokenContract {
    ...
    async fn send_rewards(&mut self, round_id: u64) -> Result<(), String> {
        // Get the round winners
        let winners = self.state.get_round_winners(round_id).await
            .map_err(|e| format!("Failed to get round winners: {:?}", e))?;
        
        // Collect unique source chain IDs to send notify messages only once per chain
