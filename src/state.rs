...
fn calculate_winnings_proportional(bet_amount: Amount, winner_pool: Amount, total_prize_pool: Amount) -> Amount {
    ...
    // Using saturating_mul to prevent overflow
    let numerator = bet_u128.saturating_mul(total_prize_pool_u128);
    let winnings_u128 = numerator / winner_pool_u128;
...
impl NativeFungibleTokenState {
    ...
    /// Note: This method should be used when the balance is already reflected in the state
    ...
    pub async fn claim_winnings(&mut self, round_id: u64, owner: AccountOwner) -> Result<Amount, String> {
        ...
        // Get the bet for this round and user
        let bet_key = (round_id, owner.clone());
        let mut bet = self.resolved_bets.get(&bet_key).await
            .map_err(|e: ViewError| format!("Failed to get bet: {:?}", e))?
            .ok_or("No bet found for this user")?
            .clone();
        ...
        if bet.prediction != correct_prediction {
            return Err("Incorrect prediction".to_string());
        }
        ...
        let current_balance = self.accounts.get(&owner).await
            .map_err(|e: ViewError| format!("Failed to get balance: {:?}", e))?
            .unwrap_or_default();
        let new_balance = current_balance.saturating_add(winnings);
        self.accounts.insert(&owner, new_balance)
            .map_err(|e: ViewError| format!("Failed to update balance: {:?}", e))?;
    ...
    /// Store a pending cross-chain bet
    pub async fn store_pending_cross_chain_bet(&mut self, source_chain_id: String, source_owner: AccountOwner, target_owner: AccountOwner, amount: Amount, prediction: Prediction) -> Result<(), ViewError> {
        self.pending_cross_chain_bets.insert(&(source_chain_id, source_owner), (target_owner, amount, prediction))
    }
